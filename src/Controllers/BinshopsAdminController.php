...
class BinshopsAdminController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $posts = BinshopsPostTranslation::with('post')->whereHas('post', function($query){ 
                    return $query->where('user_id', \Auth::user()->id);
                })->
                orderBy("post_id", "desc")->where('lang_id', $language_id)
                ->paginate(10);
    ...
    public function create_post(Request $request)
    {
        ...
        $ts = BinshopsCategoryTranslation::where("lang_id",$language_id)
